...
if (!defined('PRODUCTO_INCLUDED')) {
    ...
    class Producto
    {
        ...
        {
            $sql = "SELECT 
                p.idproducto, p.idproveedor, p.nomproducto, p.unimed, 
                p.stock, p.preuni, p.cosuni, p.idcategoria, p.estado,
                pr.nomproveedor AS proveedor_nombre,
                c.nomcategoria AS categoria_nombre
            FROM productos p 
            LEFT JOIN proveedores pr ON p.idproveedor = pr.idproveedor 
            LEFT JOIN categorias c ON p.idcategoria = c.idcategoria 
